...
describe('DELETE /comments/:id', () => {
    // error
    test('should return HTTP code 403 when delete comment forbiden', async () => {
    ...
    test('should return HTTP status code 200', async () => {
        const { body } = await request(app)
          ...
          .set('Authorization', `Bearer ${userToken2}`)
          .expect(200);
        expect(body.message).toMatch(/Your Comment has been successfully deleted/i)
    });

    test('should return HTTP status code 200 And coment has deleted', async () => {
        const { body } = await request(app)
            .delete('/comments/1')
            .set('Authorization', `Bearer ${userToken1}`)
            .expect(200);
